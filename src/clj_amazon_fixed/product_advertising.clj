(ns clj-amazon-fixed.product-advertising
...
(defn- parse-results [xml]
  ...
  (case (:tag xml)
    ...
    :Width  [:width (first (:content xml))]
    :Height  [:height (first (:content xml))]
    :EditorialReviews [:review-content (-> ((fn [c] (filter #(= (:tag %) :Content) c)) (-> xml :content first :content )) first :content first)]
